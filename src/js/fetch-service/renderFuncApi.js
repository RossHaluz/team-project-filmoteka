...
/* AddQueue */
refs.backdrop.addEventListener('click', addQueueBtnClick);

function addQueueBtnClick(e) {
  if (
...
function handlerSetLocalStorage(id) {
  const btn = document.querySelector('.modal__queue-btn');
  const { pushFilm, movies } = localStorageServiceFilms.setFilms(id);
  if (pushFilm) {
...
function createMarkup({
  ...
  if (movieStore.indexOf(id) === -1) {
    activeText = 'ADD TO QUEUE';
  } else {
    activeText = 'REMOVE FROM QUEUE';
  }
  const markup = `<div class ="modal__img-box">
      <img
        class="modal__image"
      ...
      <div class="modal__text-content">
        ...
        <div class="modal__btns">
          ...
          <button type="button" class="modal__queue-btn" data-id = ${id}>${activeText}</button>
        </div>
      </div>
  `;
